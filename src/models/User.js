...
const userSchema = new mongoose.Schema({
  ...
  // password is required for email/password accounts but not for Google OAuth accounts.
  // The validator checks: if this document has a googleId, password is not required.
  password: {
    type:     String,
    select:   false,
    minlength: 6,
    required: function () {
      // Only required when there is no googleId (i.e. not a Google OAuth account)
